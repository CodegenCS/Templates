...
///
/// Usage: dotnet-codegencs template run DapperDalPocos.cs <DbSchema.json> <Namespace> [-p:SingleFile] [-p:AddTableAttribute <true/false>] [-p:AddKeyAttribute <true/false>] [-p:AddKeyAttribute <true/false>] [-p:GenerateEqualsHashCode <true/false>] [-p:TrackPropertiesChange <true/false>]
/// e.g.:  dotnet-codegencs template run DapperDalPocos.cs AdventureWorksSchema.json MyPOCOs -db false
///
/// Arguments:
...
public class DapperPOCOGenerator : ICodegenMultifileTemplate<DatabaseSchema>
{
    ...
    public static void ConfigureCommand(Command command)
    {
        ...
        command.AddOption(new Option<string>("-p:CrudClass") { Arity = ArgumentArity.ExactlyOne, Description = "Class name for Data Access Layer" });
        command.AddOption(new Option<string>("-p:CrudFile") { Arity = ArgumentArity.ExactlyOne, Description = "File name for Data Access Layer" });
        command.AddOption(new Option<string>("-p:CrudNamespace") { Arity = ArgumentArity.ExactlyOne, Description = "Namespace for Data Access Layer" });
    }
    ...
    public class DapperPOCOGeneratorOptions : IAutoBindCommandLineArgs
    {
        ...
        /// <summary>
        /// Class name for Data Access Layer
        /// </summary>
        public string CrudClass { get; set; } = "DataAccessLayer";

        /// <summary>
        /// File for Data Access Layer
        /// </summary>
        public string CrudFile { get; set; } = "DataAccessLayer.cs";

        /// <summary>
        /// Namespace for Data Access Layer. If not defined will be the same as POCOs
        /// </summary>
    ...
    public void Render(ICodegenContext context, DatabaseSchema schema)
    {
        ...
        _logger.WriteLineAsync($"Generating CRUD DAL {ConsoleColor.Yellow}'{_options.CrudFile}'{PREVIOUS_COLOR}...");
    ...
    private Action<Table, ICodegenTextWriter> GenerateCrudSave = (table, w) =>
    {
        ...
        w.Write($$"""
            ...
            /// </summary>
            public virtual void Save({{GetClassNameForTable(table)}} e, IDbTransaction transaction = null, int? commandTimeout = null)
            {
                if ({{pkHasNoValue}})
    ...
    private FormattableString GenerateCrudInsert(Table table)
    {
        ...
        return $$"""
            ...
            /// Saves new record
            /// </summary>
            public virtual void Insert({{GetClassNameForTable(table)}} e, IDbTransaction transaction = null, int? commandTimeout = null)
            {
                string cmd = @"
                ...
                {{IIF(identityCol != null,
                        () => $$"""e.{{() => GetPropertyNameForDatabaseColumn(table, identityCol?.ColumnName)}} = this.Query<{{GetTypeDefinitionForDatabaseColumn(table, identityCol)}}>(cmd + "SELECT SCOPE_IDENTITY();", e, transaction, commandTimeout: commandTimeout).Single();""",
    ...
    private Action<Table, DapperPOCOGeneratorOptions, ICodegenTextWriter> GenerateCrudUpdate = (table, _options, w) =>
    {
        ...
        w.Write($$"""
            ...
            /// </summary>
            public virtual void Update({{GetClassNameForTable(table)}} e, IDbTransaction transaction = null, int? commandTimeout = null)
            {
                string cmd = @"
